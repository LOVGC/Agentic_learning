...
3. **Use the output tests** (from [CHECK-FORMAT.md](./CHECK-FORMAT.md)):
   ...
   - **First-principles probe** — pose a fundamental "why does this even work / why do we believe this is possible" question that targets the *premise beneath* the content, not the content itself. Open-ended; reason together; the stuck point is where they swap a name for a reason. This is the **framework lesson's signature** test (its "fundamental questions" block).
...
- **The first lesson is always the framework lesson.** Before any narrow unit, teach the **Basics** — the single framework of which the rest of the topic is a variation (the `MAP.md` "framework" section, made see-able). This is `lessons/0001`, and **Unit 0** of the syllabus for a complex topic. It is the one deliberately **wide, shallow** lesson: its win is *orientation*, not mastery — the scaffold every later lesson hangs a bone on, the basic mental model the learner thinks *with* from then on. Make it maximally visual (the big-picture diagram they carry forward) and compress hard to the skeleton — resist teaching the details, they come later. (This is the explicit exception to "one tightly-scoped thing" — every *other* lesson stays narrow and deep.)
- **The framework lesson ends in two layers of output, never recognition alone.** Close it with both:
  1. **The fast layer** — the explain-it-back (*state or draw the framework in your own words*) plus any quick recall/classify quiz. This checks the structure landed. Passing the explain-back is the gate to the first real unit.
  2. **The depth layer — fundamental questions.** A visually distinct block of **1–3 open, premise-level questions** that probe *why the field's central bet works at all* — e.g. for feature selection, *"Why should we believe a handful of columns can predict the target at all?"* Present them in the HTML with **no multiple choice and no answer revealed**; the real work happens in chat, where you ask the question, let the learner reason, and push with more "why"s until they hit bedrock (a **First-principles probe** — see [CHECK-FORMAT.md](./CHECK-FORMAT.md)). A classify-the-method quiz only tests whether the learner can apply a *label* — the "names ≠ knowing" trap; the fundamental questions are what test whether they grasp the foundation the labels sit on.
     - **What makes a question fundamental** (so these are deep, not superficial dressed up as deep): it targets the **premise/assumption the field rests on**, not the content just taught; it **can't be answered by reciting the lesson**; it often has **no single settled answer** and touches *why the field exists* (`MISSION.md`) and its open problems (`MAP.md` landscape); it's the question a **curious child or Feynman** would ask — disarmingly simple, hard to answer. Frame them as **playful invitations to wonder**, not an exam.
